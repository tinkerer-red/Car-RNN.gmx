///draw_neurons_animated(x1,y1.x2.y2)

// This function will show a live example of the calling instance's data and how it's equated for the frame.
...
for(var i = 0; i < neuron_count; i++){
    var weight = clamp(neurons[i], -1, 1)
...
for(var i = 0; i < output_count; i++){
    var weight = clamp(outputs[i], -1, 1)
...
//Draw Input Layer to Hidden Layer weight
for(var i=0; i<hidden_layer; i++){    
  for(var j=0; j<input_layer; j++){
    var weight = clamp(inputs[i]*weights[i, j], -1, 1)
...
for(var i=0; i<output_layer; i++){
  for(var j=0; j<hidden_layer; j++){
    var weight = clamp(neurons[i]*output_weights[i, j], -1, 1)
